...
<body id="top">
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                @if($transaksi->produk)
                                    <img src="{{ asset('storage/' . $transaksi->produk->gambar_produk) }}" alt="{{ $transaksi->nama_produk }}" class="product-image">
                                @else
                                    <img src="https://via.placeholder.com/50" alt="{{ $transaksi->nama_produk }}" class="product-image">
                                @endif
                            </td>
                            <td>{{ $transaksi->nama_produk }}</td>
                            <td>{{ number_format($transaksi->harga_produk, 2) }}</td>
                            ...
                            <td>
                                <button class="review-button" data-id="{{ $transaksi->produk->id_produk }}">Beri Ulasan</button> <!-- Tombol beri ulasan -->
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
    ...
    <script>
        document.querySelectorAll('.star-rating').forEach(starRating => {
            ...
            const ratingValueInput = starRating.nextElementSibling; // Mengambil input hidden di sebelahnya
            ...
            stars.forEach((star, index) => {
                star.addEventListener('click', () => {
                    ...
                    ratingValueInput.value = currentIndex; // Menyimpan nilai rating yang dipilih
                });
            });
        });
        ...
        document.querySelectorAll('.review-button').forEach(button => {
            button.addEventListener('click', () => {
                const ratingValueInput = button.closest('tr').querySelector('.rating-value');
                ...
                .then(data => {
                    if (data.success) {
                        // Menampilkan SweetAlert2 untuk rating berhasil
                        Swal.fire({
                            icon: 'success',
                            title: 'Rating Berhasil!',
                            text: 'Rating Anda telah disimpan.',
                            confirmButtonText: 'Ok'
                        });
                    } else {
                        // Menampilkan SweetAlert2 untuk rating gagal
                        Swal.fire({
                            icon: 'error',
                            title: 'Gagal!',
                ...
                .catch(error => {
                    ...
                    // Menampilkan SweetAlert2 untuk error dalam pengiriman
                    Swal.fire({
                        icon: 'error',
